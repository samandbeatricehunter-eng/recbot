...
'      values (${interaction.guildId}, ${interaction.user.id}, ${ids}, ${"commissioners_selected"})',
...
write('src/commands/new-server-setup.ts', commandFile);
write('src/lib/new-server-setup-handlers.ts', handlersFile);

patch('src/lib/command-list.ts', (s) => {
  if (!s.includes('new-server-setup.js')) {
    if (s.includes('import * as actions from "../commands/actions.js";')) {
      s = s.replace(
        'import * as actions from "../commands/actions.js";',
        'import * as actions from "../commands/actions.js";\nimport * as newServerSetup from "../commands/new-server-setup.js";'
      );
    }
    if (s.includes('actions,') && !s.includes('newServerSetup,')) {
...
patch('src/deploy-commands.ts', (s) => {
  if (s.includes('const allowed = new Set(["menu"]);')) {
    return s.replace('const allowed = new Set(["menu"]);', 'const allowed = new Set(["menu", "new-server-setup"]);');
  }
  return s;
});
...
patch('src/events/interactionCreate.ts', (s) => {
  if (!s.includes('new-server-setup-handlers.js')) {
    const importLine = 'import { handleGameOfficeInteraction } from "../lib/game-office-handlers.js";';
    if (s.includes(importLine)) {
      s = s.replace(
        importLine,
        importLine + '\nimport { handleNewServerSetupInteraction } from "../lib/new-server-setup-handlers.js";'
      );
    } else {
      s = 'import { handleNewServerSetupInteraction } from "../lib/new-server-setup-handlers.js";\n' + s;
    }
  }
  ...
  if (!s.includes('handleNewServerSetupInteraction(interaction as any)')) {
    ...
    if (s.includes(marker)) {
      ...
      console.warn('Could not find autocomplete marker in interactionCreate.ts; handler import was added but routing may need manual placement.');
    }
  }
  return s;
});
