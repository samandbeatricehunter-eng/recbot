...
const root = process.cwd().endsWith(path.sep + 'scripts') ? path.dirname(process.cwd()) : process.cwd();
const target = path.join(root, 'src', 'lib', 'mca-raw-storage.ts');
fs.mkdirSync(path.dirname(target), { recursive: true });

const content = `import { db } from "@workspace/db";
...
console.log('Created/updated:', path.relative(root, target));
console.log('Next: import and call storeMcaRawPayload(...) inside src/lib/league-data-handlers.ts import paths.');
