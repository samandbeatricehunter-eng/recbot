  ...
  if (action === "violation_confirm") {
    ...
    if (violation.discordId) {
      ...
      if (confirmedCount >= 2) {
        ...
        const repeatViolCommChannelId = await getGuildChannel(interaction.guildId!, CHANNEL_KEYS.COMMISSIONER);
...
async function handleModal(interaction: ModalSubmitInteraction) {
  ...
  if (action === "interview_modal") {
    ...
    try {
      const interviewDenialCommChannelId = await getGuildChannel(interaction.guildId!, CHANNEL_KEYS.COMMISSIONER);
