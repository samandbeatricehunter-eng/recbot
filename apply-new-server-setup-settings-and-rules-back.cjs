...
const root = findRoot();
const setupPath = path.join(root, 'src', 'lib', 'new-server-setup-handlers.ts');
const adminOpsPath = path.join(root, 'src', 'lib', 'admin-operations-handlers.ts');
...
backup(setupPath, 'settings-rules-back');
fs.writeFileSync(setupPath, setupCode, 'utf8');
console.log('Rewrote src/lib/new-server-setup-handlers.ts');

if (fs.existsSync(adminOpsPath)) {
  backup(adminOpsPath, 'setup-back-routing');
  let admin = fs.readFileSync(adminOpsPath, 'utf8');
  if (!admin.includes('new-server-setup-handlers.js')) {
    admin = admin.replace(
      'import { buildLeagueDataMainMenu } from "./league-data-handlers.js";',
      'import { buildLeagueDataMainMenu } from "./league-data-handlers.js"; import { buildNewServerSetupIntro, buildNewServerSetupRows, setupIsCompleted } from "./new-server-setup-handlers.js";'
    );
  }
  const oldBlock = /if \(id === "ao_hub_back"\) \{[\s\S]*?return true; \}/;
  const newBlock = [
    'if (id === "ao_hub_back") {',
    '    const setupComplete = await setupIsCompleted(guildId).catch(() => true);',
    '    if (!setupComplete) {',
  ...
  if (oldBlock.test(admin)) {
    admin = admin.replace(oldBlock, newBlock);
    fs.writeFileSync(adminOpsPath, admin, 'utf8');
    console.log('Patched admin rules Back to Hub behavior for setup context.');
  } else {
    console.warn('Could not find ao_hub_back block in admin-operations-handlers.ts. No back-button patch applied.');
  }
}
